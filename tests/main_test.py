...
from dotnet_pre_commit_mirror_maker import main
...
def test_main_defaults_entry_to_package_name(mock_make_repo):
    ...
    assert mock_make_repo.call_args[1]["entry"] == "csharpier"
...
def test_main_defaults_id_to_entry(mock_make_repo):
    ...
    assert mock_make_repo.call_args[1]["id"] == "csharpier"
...
def test_main_with_args(mock_make_repo):
    assert not main.main(
        (
            ...
            "--files-regex",
            r"\.cs$",
            r"--args=--check,--write-stdout",
        )
    )
    expected = '["--check", "--write-stdout"]'
